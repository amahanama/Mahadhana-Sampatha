...
$_SESSION['login_user']= $username;  // Initializing Session with value of PHP Variable
  ...
  if(remove_junk(ucfirst(current_user()['name'])) == null || remove_junk(ucfirst(current_user()['name']))== ""){	  
...
<script type="text/javascript">
	...
	function validateNIC(nic){
		if(nic.length != 0){
			if(nic.length == 10){	
				...
				alert("lastLetter "+lastLetter);
				if(!(lastLetter.toLowerCase() == "x" || lastLetter.toLowerCase() == "v")){
					alert("Invalid NIC");
					document.getElementById('nic').value="";
				}
			} else if(nic.length == 12){
				
				if(nic.match(/^[0-9]+$/) == null){
					alert("Invalid NIC");
					document.getElementById('nic').value="";
				}
	...
	function validate_mobile_nos(){
		if(document.getElementById('hm_tp').value.length != 0 && document.getElementById('mobile_no').value.length != 0){
			if(document.getElementById('hm_tp').value == document.getElementById('mobile_no').value){
				alert("Mobile No and Home Telephone No can not be similar.");
   ...
   */
   ...
   if(empty($errors)){
		...
		*/
		...
		if(($P_data != null)) {	
			$session->msg('d',' Panel Member is already exist.');
		...
		$p_count=find_panel_count_by_nic("panel_member",$nic);
		
		if($p_count['c'] > 0){
			$session->msg('s',"Panel Member is already exist!".$p_count);
			redirect('add_panel.php', false);
		}else{
	  ...
	  <form action="add_panel.php" method = "POST" enctype = "multipart/form-data">
        <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="full-name" placeholder="Full Name" onkeydown="return /^[a-zA-Z ]+$/i.test(event.key)">
            </div>
            <div class="form-group">
                <label for="nic">NIC</label>
                <input type="text" class="form-control" name="nic" id="nic" placeholder="NIC" onKeyPress="if(this.value.length==12) return false;" onblur="validateNIC(this.value)">
            </div>
            
            
			<div class="form-group">
              <label for="hm_tp">Home Telephone No</label>
                 <input class="form-control" name="hm_tp" id="hm_tp" placeholder="Home Telephone"  type="text" onKeyPress="if(this.value.length==10) return false;if ( isNaN(this.value + String.fromCharCode(event.keyCode) )) return false;" onblur="validate_mobile_nos();">
            </div>
		 ...
		 <div class="col-md-6">
            <div class="form-group">
                ...
                <input type="text" class="form-control" name ="designation"  placeholder="Designation" onkeydown="return /^[a-zA-Z ]+$/i.test(event.key)" >
            </div>
            <div class="form-group">
			...
			<div class="form-group">
              <label for="mobile_no">Mobile No</label>
                 <input class="form-control" name="mobile_no" id="mobile_no" placeholder="Mobile No"  type="text" onKeyPress="if(this.value.length==10) return false;if ( isNaN(this.value + String.fromCharCode(event.keyCode) )) return false;" onblur="validate_mobile_nos();">
            </div>
			<div class="form-group">              			   
